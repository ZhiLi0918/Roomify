...
const validationSchema = Yup.object().shape({
  selectedRooms: Yup.array()
    .min(1, "You must select at least one room.")
...
export const Reserve = ({ setOpen, hotelId }) => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      await Promise.all(
        values.selectedRooms.map((roomId) => {
          return axios.put(`/room/availability/${roomId}`, { dates: alldates });
        })
      );
      setOpen(false);
      navigate("/");
        ...
        >
          {({ values, errors, touched, isSubmitting }) => (
            <Form>
              ...
              {touched.selectedRooms && errors.selectedRooms && (
                <div style={{ color: "red" }}>{errors.selectedRooms}</div>
              )}
              <RButton type="submit" disabled={isSubmitting}>
